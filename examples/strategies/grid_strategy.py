...
class GridStrategy(CtaTemplate):
    ...
    def on_15min_bar(self, bar):
        ...
        self.am.update_bar(bar)  # 更新K线序列

        if not self.am.inited:  # 如果K线序列还没有初始化完成，就不进行交易逻辑
            return

        # 如果网格价格列表为空，就根据当前价格生成网格价格列表
        if not self.grid_prices:
            price = bar.close_price  # 获取当前收盘价
        ...
        if bar.close_price > grid_price:  # 如果当前价格高于网格价格
        ...
        elif bar.close_price < grid_price:  # 如果当前价格低于网格价格
